...
CREATE OR REPLACE PROCEDURE mechanic_bonus(
	mechanic INTEGER,
	BonusAmount DECIMAL
)
LANGUAGE plpgsql
AS $$
BEGIN
	-- Add bonus hours to mechanic hoursworked
	UPDATE HoursWorked
	SET hoursworked = hoursworked + BonusAmount
	WHERE mechanicid = mechanic
...
CREATE OR REPLACE procedure price_inflation(
	carpart INTEGER,
	InflationSpread DECIMAL
)
LANGUAGE plpgsql
AS $$
BEGIN
	-- Add bonus hours to mechanic hoursworked
	UPDATE CarPart_Ticket
	SET price = price + InflationSpread
	WHERE CarPartID = carpart;
